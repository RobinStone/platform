...
<section class="message">
    ...
    <h2 class="h2">Укажите пункт приёма СДЭК, из которого Вы будете отправлять посылку ? (<?=$city?>)</h2>
    <div id="map"></div>
<script>
    myMap = null;
                ...
                function init() {
                    ...
                    setTimeout(function() {
                        ...
                        for(let i in points) {
                            ...
                            console.dir(loc);
                            add_placemark(loc.latitude, loc.longitude, label_img, "<br><b>"+loc.address+"</b>", points[i].code);
                        }
                    }, 100);
                }
                ...
                function add_placemark(lat, lon, label_img, hint='', baloon='') {
                    ...
                    myPlacemark.events.add('click', function (e) {
                        info_qest(undefined, function() {
                            // say(baloon, 2);
                            BACK('cdek', 'set_start_point_cdek', {point: baloon, order_id: <?=$order_id?>}, function(mess) {
                                mess_executer(mess, function(mess) {
                                    close_popup('map_cdek_from');
                                    say('Точка отправки посылки - установлена');
                                });
                            });
                        }, function() {
                            location.reload();
                        }, 'Подтвердите, что вы собираетесь отправлять посылку из пункта СДЭК, по адресу<br><b>'+hint+'</b> ?');
                        console.log(hint);
                    });
